...
- 机场流量通过订阅链接响应头 `subscription-userinfo` 解析（Clash / V2Ray 订阅通用）
...
> 注意：部分机场（如 flowercloud）的订阅更新开关默认关闭，查询流量前需先到官网临时打开开关。
...
本项目不收集任何数据。API Key 和订阅链接仅保存在本机 `%APPDATA%\DesktopWidget\config.json`，仅用于向 OpenRouter 和你的机场官方接口发起请求。
